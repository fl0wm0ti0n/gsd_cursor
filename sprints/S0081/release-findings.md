# Release findings — S0081 / US-0092 (Full-autonomy outer driver)
...
- **dec_id**: DEC-0078 (full_autonomy outer driver + UAT probe lib + stop matrix; composes US-0088, DEC-0062, DEC-0047, DEC-0048)
- **decision_refs**: DEC-0078, DEC-0039, DEC-0038, DEC-0029, DEC-0018, DEC-0040

## Pre-release preflight (re-run on fresh release context)

| gate | command | result |
|------|---------|--------|
| us0092_contract | `python -m pytest tests/auto_command_contract_test.py -q -k us0092` | **PASS** (9 passed, 18 subtests) |
...
| readme_feature_coverage (3f) | `python scripts/validate_readme_feature_coverage.py --repo . --enforce` | **FAIL** — `README_FEATURE_COVERAGE_GAP:BUG-0009`; `README_FEATURE_COVERAGE_INPUT_INVALID` (BUG-0010, BUG-0011, US-0091 `user_visible` unset); `README_FEATURE_COVERAGE_PARITY_FAIL` — pre-existing post-S0077 drift; disjoint from DEC-0078 |

All US-0092-scoped preflight gates green on fresh release context.

## Test baselines (consumed from QA cycle 1 + verify-work; see handoffs/qa_to_release.md)

- **Canonical check-in** (`tests/run-tests.ps1`): **Pass=808 / Fail=14** (`tests/report.md` Timestamp=2026-06-06T17:10:23Z). Fail=14 unchanged (disjoint from DEC-0078).
- **`pytest -k us0092`**: 9 passed (release re-run).
- **`auto_outer_driver.py --self-test`**: `[AUTO_OUTER_DRIVER_SELF_TEST_OK]`.
- **`uat_probe_lib.py --self-test`**: `[UAT_PROBE_LIB_SELF_TEST_OK]`.
- **`check_intake_template_parity.py --scope=us-0092`**: `[INTAKE_TEMPLATE_PARITY_OK]`.
...
| readme_feature_coverage (3f) | observation | Live `--enforce` fails on BUG-0009 gap + `user_visible` unset rows + README parity — **disjoint from US-0092**; canonical pass recorded at S0077 release |

## Release gate chain (US-0039 / DEC-0019)

| gate | verdict | reason_code | evidence_refs |
|------|---------|-------------|---------------|
| check-in_test | pass | - | `tests/report.md` (808/14; 14 pre-existing disjoint); `pytest -k us0092` 9 passed; `sprints/S0081/qa-findings.md` |
| qa | pass | - | `sprints/S0081/qa-findings.md` (cycle 1 PASS; no blockers) |
| uat | pass | - | `sprints/S0081/uat.json`, `sprints/S0081/uat.md` (10/10 PASS) |
...
| readme_feature_coverage_3f | observation | - | post-S0077 drift; disjoint from US-0092 |
| bug_validate | pass | - | `[BUG_VALIDATION_OK]` pre- and post-release-write |
| finalization | pass | - | this file, `handoffs/releases/S0081-release-notes.md`, `handoffs/release_queue.md`, `handoffs/release_notes.md`, `docs/product/backlog.md`, `docs/product/acceptance.md`, `docs/engineering/status-normalization-report.md`, `docs/engineering/state.md` |

## Canonical release artifact policy (applied)

- `handoffs/releases/S0081-release-notes.md` — authored.
- `handoffs/release_queue.md` row `S0081` — `ready` → `released`.
- `handoffs/release_notes.md` — legacy pointer updated (latest released sprint = S0081).
- `sprints/S0081/release-findings.md` — this file.
- `docs/product/backlog.md` `## US-0092` — status `OPEN` → `DONE`; AC-1..AC-10 `[x]`; `release_notes` block appended.
- `docs/product/acceptance.md` — US-0092 portfolio row `[ ]` → `[x]`.
- `docs/engineering/status-normalization-report.md` — US-0092 delta row appended.
...
- **README feature coverage drift** — BUG-0009 gap + `user_visible` metadata + README/template parity; remediate before next enforce-gated release.
- **Portfolio** — **0 OPEN** stories after US-0092 closure; backlog drain budget **2** remaining.
