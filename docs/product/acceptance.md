...
- [ ] US-0111: Release-trigger-driven version changelog derivation — trigger adapter system (GitHub/npm/git tag/manual), version diff, atomic [Unreleased] promotion, sovereign ledger integration; compose with US-0100/US-0103 (12 ACs)
- [ ] US-0112: Ship model-catalog example presets on install/upgrade — all eight `.cursor/model-catalog.local.example*.json` files in installer manifest; missing/upgrade framework delivery; never overwrite `model-catalog.local.json`; runbook + contract tests + parity (8 ACs)
