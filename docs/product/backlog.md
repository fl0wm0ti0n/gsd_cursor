...
- Acceptance:
  - [ ] AC-1: **Syntax contract**: documented **`/auto`** invocations (exact spellings **architecture-locked**) for **all OPEN bugs** and **single `BUG-####`**; invalid ids / **DONE** bugs → deterministic **fail-closed** codes (**`# US-0087`** vocabulary).
  - [ ] AC-2: **Scratchpad + `template/`**: new flags (names **architecture-locked**) e.g. bug-target mode + optional **max-bugs-per-run** + **on-block** stop/skip — **default-off** preserves today’s behavior.
  - [ ] AC-3: **`docs/engineering/auto-orchestration-reference.md`** + **active + `template/`** **`.cursor/commands/auto.md`**: precedence rules (**explicit argv** > merged scratchpad > **`resume_brief`** > **`state.md`** fallback) updated for bug-target path; conflict with **`AUTO_BACKLOG_DRAIN=1`** documented (**one scheduler wins**).
  - [ ] AC-4: **Queue semantics**: **OPEN** bugs only, **sorted by numeric id**; **fix all** respects **`AUTO_BUG_MAX_ITEMS`** (or architecture-locked equivalent) when set; empty queue → **`AUTO_BUG_QUEUE_EMPTY`** (or locked name).
  - [ ] AC-5: **`handoffs/resume_brief.md`** + **`docs/engineering/state.md`**: each segment records **`bug_id`** (and queue position when applicable); no **`RESUME_BRIEF_STALE`** false positives for lawful bug-target runs (**`DEC-0069`** alignment).
  - [ ] AC-6: **Spawn-only**: orchestrator never executes phase work in-process (**`BUG-0006`** / **`US-0069`** unchanged).
  - [ ] AC-7: **Tests**: **`tests/auto_command_contract_test.py`** (or successor) asserts markers / fixture rows for bug-target argv and policy conflict cases.
  - [ ] AC-8: **Architecture** **`# US-0087`**: reason codes, flag names, interaction matrix (**`US-0044`**, **`DEC-0069`**, **`US-0070`**, **`US-0079`**).
  - [ ] AC-9: **Runbook** pointer: operator recipe “targeted bug auto drain” (**`docs/engineering/runbook.md`**) when **`US-0087`** enabled.
