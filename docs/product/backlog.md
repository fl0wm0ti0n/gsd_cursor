...
- Acceptance:
  - [ ] AC-1: **Scratchpad contract** — new keys documented in **active + `template/`** scratchpad surfaces (and **example** where applicable): at minimum **`CAVEMAN_MODE=0|1`** (default **0**) and **`CAVEMAN_LEVEL=lite|full|ultra`** (exact enum **architecture-locked**); optional keys reserved for **US-0090** may appear as **documented no-ops** until that story ships.
  - [ ] AC-2: **Default-off parity** — with **`CAVEMAN_MODE=0`** (or absent key treated as **0** per architecture), **no** change to normative command strings, gate ordering, spawn-only language (**BUG-0006**), or existing contract tests beyond **additive** assertions.
  - [ ] AC-3: **Cursor behavior pack** — **rules and/or `.cursor/skills`** (and command cross-links if needed) implement terse style when enabled; **code blocks**, **paths**, **AC checklists**, and **reason codes** remain **literal** (no “caveman” garbling of machine-verifiable artifacts).
  - [ ] AC-4: **`TOKEN_PROFILE` composition** — **`docs/engineering/auto-orchestration-reference.md`** and/or **`docs/engineering/runbook.md`** state how **`TOKEN_PROFILE`** and **`CAVEMAN_*`** interact (**orthogonal** unless architecture documents one explicit precedence matrix).
  - [ ] AC-5: **Operator control** — documented phrases to **enable** / **disable** Caveman voice for a session (align with common **stop caveman** / **normal mode** patterns); deterministic behavior when toggled.
  - [ ] AC-6: **Tests** — extend **`tests/auto_command_contract_test.py`** (or add a focused test module) so **default-off** and **key documentation markers** (or rule files) are **regression-locked**.
  - [ ] AC-7: **`architecture.md`** — new section **`# US-0089`** with boundaries vs **US-0090**, vs **US-0053**/**US-0080**, and **non-goals** (e.g. no mandatory whole-repo file rewrite in this story).
  - [ ] AC-8: **Template parity** — every touched **`.cursor/`** / **`docs/engineering/`** surface mirrored under **`template/`** per **US-0017** expectations.
- Boundaries:
...
- Acceptance:
  - [ ] AC-1: **Gating** — compression **inactive** unless **`CAVEMAN_MODE=1`** **and** an explicit **`CAVEMAN_COMPRESS_INPUT`** policy is **on** (exact enum **architecture-locked**), default **off**.
  - [ ] AC-2: **Originals** — every write creates or updates a deterministic **sidecar original** (path pattern **architecture-locked**), never **destructive-only** loss.
  - [ ] AC-3: **Deny list** — built-in refusals for **`.env`**, `**/*intake_evidence*/**`, canonical **`docs/product/backlog.md`**, **`docs/product/acceptance.md`**, and **`docs/engineering/state.md`** unless architecture moves this list behind an explicit **decision gate** (**default PO expectation: always deny**).
  - [ ] AC-4: **Scope** — **`CAVEMAN_FILE_SCOPE`** (or successor) maps to **allow** sets (**docs only**, **explicit glob**, **etc.**) with deterministic diagnostics on violation (**reason codes**).
  - [ ] AC-5: **Operator UX** — runbook section: **how to run**, **how to revert**, **how to verify** no canonical artifact drift; **dry-run** mode if architecture selects it.
  - [ ] AC-6: **Tests** — unit/integration tests on **path guards**, **scope parser**, and **idempotency** (no double-corruption).
  - [ ] AC-7: **`architecture.md`** — section **`# US-0090`** links **`# US-0089`**, **US-0053**, **US-0085**, and documents **forbidden surfaces**.
  - [ ] AC-8: **Template parity** for any new script references, scratchpad keys, and runbook excerpts.
- Boundaries:
